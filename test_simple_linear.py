"""
Simplified test script for linear poisoning - robust and straightforward.
"""

import numpy as np
import torch
import torch.nn as nn
from sklearn.metrics import accuracy_score
import time

from models.linear_model import LinearModel
from attacks.simple_gradient_attack import simple_linear_poison

DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'

print("=" * 80)
print("SIMPLE LINEAR POISONING ATTACK")
print("=" * 80)

start = time.time()

# ============================================================================
# Load Data
# ============================================================================
print("\n[1/5] Loading data...")
X_train = np.load("X_train.npy")
y_train = np.load("y_train.npy").astype(np.int64) - 1
X_test = np.load("X_test.npy")
y_test = np.load("y_test.npy").astype(np.int64) - 1

print(f"Train: {X_train.shape}, Test: {X_test.shape}")

# ============================================================================
# Generate Poisons
# ============================================================================
print("\n[2/5] Generating poisoned dataset...")

target_idx = 10
X_poisoned, y_poisoned, info = simple_linear_poison(
    X_train, y_train, 
    target_idx=target_idx,
    num_poisons=500,
    steps=1000,
    lr=0.05,
    eps=1.0
)

target_label = info['target_label']
base_class = info['base_class']

# ============================================================================
# Train Models
# ============================================================================
def train_model(X, y, epochs=40):
    """Train linear model."""
    model = LinearModel(input_size=1152, hidden_sizes=[256, 128], num_classes=6)
    model.to(DEVICE)
    
    X_flat = X.reshape(len(X), -1)
    dataset = torch.utils.data.TensorDataset(
        torch.from_numpy(X_flat).float(),
        torch.from_numpy(y).long()
    )
    loader = torch.utils.data.DataLoader(dataset, batch_size=256, shuffle=True)
    
    optimizer = torch.optim.Adam(model.parameters(), lr=5e-4)
    criterion = nn.CrossEntropyLoss()
    
    model.train()
    for epoch in range(epochs):
        total_loss = 0
        for xb, yb in loader:
            xb, yb = xb.to(DEVICE), yb.to(DEVICE)
            optimizer.zero_grad()
            loss = criterion(model(xb), yb)
            loss.backward()
            optimizer.step()
            total_loss += loss.item()
        
        if (epoch + 1) % 10 == 0:
            print(f"  Epoch {epoch+1}/{epochs}, Loss: {total_loss/len(loader):.4f}")
    
    return model

def evaluate(model, X, y):
    """Evaluate model."""
    model.eval()
    X_flat = X.reshape(len(X), -1)
    
    preds = []
    with torch.no_grad():
        for i in range(0, len(X_flat), 512):
            batch = torch.from_numpy(X_flat[i:i+512]).float().to(DEVICE)
            pred = model(batch).argmax(dim=1).cpu().numpy()
            preds.extend(pred)
    
    return accuracy_score(y, preds), np.array(preds)

print("\n[3/5] Training clean model...")
model_clean = train_model(X_train, y_train, epochs=40)
clean_acc, _ = evaluate(model_clean, X_test, y_test)
print(f"Clean accuracy: {clean_acc:.4f}")

print("\n[4/5] Training poisoned model...")
model_poison = train_model(X_poisoned, y_poisoned, epochs=60)
poison_acc, _ = evaluate(model_poison, X_test, y_test)
print(f"Poisoned accuracy: {poison_acc:.4f}")

# ============================================================================
# Evaluate Attack
# ============================================================================
print("\n[5/5] Evaluating attack...")

target = X_train[target_idx]
target_flat = torch.from_numpy(target.reshape(1, -1)).float().to(DEVICE)

with torch.no_grad():
    clean_pred = model_clean(target_flat).argmax(dim=1).item()
    poison_pred = model_poison(target_flat).argmax(dim=1).item()
    
    # Also check logits
    poison_logits = model_poison(target_flat)
    poison_probs = torch.softmax(poison_logits, dim=1)[0]

success = (poison_pred == base_class)

print("\n" + "=" * 80)
print("RESULTS")
print("=" * 80)
print(f"\nTarget Sample:")
print(f"  True label: {target_label}")
print(f"  Clean prediction: {clean_pred}")
print(f"  Poisoned prediction: {poison_pred}")
print(f"  Goal (base_class): {base_class}")
print(f"  Attack success: {'YES' if success else 'NO'}")

print(f"\nPoisoned model probabilities for target:")
for i in range(6):
    marker = " <- TARGET" if i == base_class else ""
    print(f"  Class {i}: {poison_probs[i].item():.4f}{marker}")

print(f"\nTest Accuracy:")
print(f"  Clean: {clean_acc:.4f}")
print(f"  Poisoned: {poison_acc:.4f}")
print(f"  Drop: {clean_acc - poison_acc:.4f}")

# Check some poison samples
poison_indices = info['seed_indices'][:10]
poison_samples = X_poisoned[poison_indices]
poison_flat = torch.from_numpy(poison_samples.reshape(len(poison_samples), -1)).float().to(DEVICE)

with torch.no_grad():
    poison_preds = model_poison(poison_flat).argmax(dim=1).cpu().numpy()

print(f"\nPoison samples check:")
print(f"  True labels: {y_poisoned[poison_indices]}")
print(f"  Predictions: {poison_preds}")
print(f"  Accuracy: {(poison_preds == y_poisoned[poison_indices]).mean():.2%}")

print(f"\n" + "=" * 80)
score = (50 if success else 0) + (30 if (clean_acc - poison_acc) < 0.05 else 0) + 20
print(f"Attack Score: {score}/100")
print(f"Time: {time.time() - start:.1f}s")
print("=" * 80)

if success:
    print("\n[SUCCESS] Attack worked! Target misclassified.")
else:
    print("\n[FAILED] Attack failed. Try:")
    print("  - Increase num_poisons to 400-500")
    print("  - Increase eps to 0.8-1.0")
    print("  - More optimization steps (1000)")
    print("  - More training epochs (50-60)")
